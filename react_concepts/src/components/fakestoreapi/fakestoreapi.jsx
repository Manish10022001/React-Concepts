...
export default function FakeStore() {
  ...
  function loadCategories() {
    axios
      ...
      .then((response) => {
        response.data.unshift("All"); //to gell "all" option
  ...
  return (
    <div className="container-fluid">
      <header className="d-flex bg-dark text-light fs-5 justify-content-between border p-2 mt-2">
        <div>
          <span className="fs-4">Fakestore</span>
        </div>
        <nav>
          <span>Electronics</span>
          <span className="mx-3">Men's Clothing</span>
          <span>Women Fashion</span>
          <span className="ms-3">Jewelery</span>
        </nav>
        <div>
